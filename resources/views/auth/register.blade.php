...
<head>
    ...
    <title>Register - Sandana TrackFlow</title>
...
<body class="h-full antialiased overflow-hidden bg-white">
    ...
    <div class="min-h-screen grid lg:grid-cols-12 bg-white">
        ...
        <div class="lg:col-span-5 flex flex-col justify-between p-8 sm:p-12 md:p-16 bg-white z-10 overflow-y-auto">
            ...
            <div class="w-full max-w-md mx-auto my-auto py-8 space-y-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <p class="text-xs sm:text-sm text-gray-500 font-medium">
                        Register a new account for Sandana TrackFlow system.
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div class="space-y-1">
                        <label for="name" class="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Full Name</label>
                        <input 
                            id="name" 
                            class="block w-full rounded-xl bg-gray-50/50 border-gray-200 focus:bg-white focus:ring-4 focus:ring-blue-900/10 focus:border-blue-900 transition-all duration-200 text-sm py-2.5 px-4 shadow-inner outline-none border" 
                            type="text" 
                            name="name" 
                            value="{{ old('name') }}" 
                            required 
                    ...
                    <div class="space-y-1">
                        <label for="role" class="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Department Role</label>
                        <select 
                            id="role" 
                            name="role" 
                            class="block w-full rounded-xl bg-gray-50/50 border-gray-200 focus:bg-white focus:ring-4 focus:ring-blue-900/10 focus:border-blue-900 transition-all duration-200 text-sm py-2.5 px-4 shadow-inner outline-none border text-gray-600 font-medium appearance-none" 
                            required>
                            <option value="" class="text-gray-400">Pilih Role Departemen</option>
                            <option value="sales" {{ old('role') == 'sales' ? 'selected' : '' }}>Sales & Marketing</option>
                            <option value="engineering" {{ old('role') == 'engineering' ? 'selected' : '' }}>Engineering (Teknik)</option>
                            <option value="finance" {{ old('role') == 'finance' ? 'selected' : '' }}>Finance & Accounting</option>
                        </select>
                        @error('role')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-1">
                        <label for="password_confirmation" class="text-[10px] text-gray-500 font-bold uppercase tracking-wider block">Confirm Password</label>
                        <input 
                    ...
                    <div class="pt-3 space-y-4">
                        ...
                        <div class="text-center">
                            <a class="text-xs text-gray-500 hover:text-blue-900 transition font-semibold" href="{{ route('login') }}">
